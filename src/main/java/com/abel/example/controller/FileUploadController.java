...
public class FileUploadController {
    ...
    @PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE, value = "upload-with-progress")
            ...
            fileService.uploadFileWithProgress(file, uniqueFileName)
                    .whenComplete((url, ex) -> {
                        if (ex != null) {
                            ...
                            log.error("uploadFileWithProgress#uploadFileWithProgress success,url:{}", url);
                        }
                    });

            return ResponseMessage.success(uniqueFileName);
        } catch (Exception e) {
            return ResponseMessage.error(500, "上传初始化失败: " + e.getMessage());
        }
    }
